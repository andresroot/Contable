...
// ESTRUCTURA MEDIO DE PAGO JSON
type mediodepagoJson struct {
	Id     string `json:"id"`
...
// MEDIO DE PAGO LISTA
func MediodepagoLista(w http.ResponseWriter, r *http.Request) {
	tmp, _ := template.ParseFiles("vista/inicio/Modulo.html",
		"vista/mediodepago/mediodepagoLista.html")
	db := dbConn()
	selDB, err := db.Query("SELECT * FROM mediodepago ORDER BY codigo ASC")
	if err != nil {
		panic(err.Error())
	}
	res := []mediodepago{}
	for selDB.Next() {
		...
		res = append(res, mediodepago{Codigo, Nombre })
	}
...
// MEDIO DE PAGO NUEVO
func MediodepagoNuevo(w http.ResponseWriter, r *http.Request) {
	...
	if Codigo == "False"{
		...
		if Elemento == "False"{
			err := db.Get(&emp, "SELECT * FROM mediodepago where codigo=$1", Codigo)
			if err != nil {
				log.Fatalln(err)
			}
		}
	...
	miTemplate, err := template.ParseFiles("vista/inicio/Modulo.html",
		"vista/mediodepago/mediodepagoNuevo.html",
		"vista/mediodepago/autocompletamediodepagocrear.html")
...
// MEDIO DE PAGO INSERTAR
func MediodepagoInsertar(w http.ResponseWriter, r *http.Request) {
	db := dbConn()
	if r.Method == "POST" {
		Codigo := r.FormValue("Codigo")
		Nombre := r.FormValue("Nombre")
		Nombre = Titulo(Nombre)
		insForm, err := db.Prepare("INSERT INTO mediodepago(codigo, nombre)VALUES($1, $2)")
		if err != nil {
		panic(err.Error())
		}
	...
	http.Redirect(w, r, "/MediodepagoLista", 301)
}

// 	MEDIO DE PAGO EXISTE
func MediodepagoExiste(w http.ResponseWriter, r *http.Request) {
	db := dbConn()
	Codigo := mux.Vars(r)["codigo"]
	var total int
	row := db.QueryRow("SELECT COUNT(*) FROM mediodepago  WHERE codigo=$1", Codigo)
	err := row.Scan(&total)
	if err != nil {
		log.Fatal(err)
	}
...
// MEDIO DE PAGO EDITAR
func MediodepagoEditar(w http.ResponseWriter, r *http.Request) {
	tmp, _ := template.ParseFiles("vista/inicio/Modulo.html",
		"vista/mediodepago/mediodepagoEditar.html")
	db := dbConn()
	Codigo := mux.Vars(r)["codigo"]
	selDB, err := db.Query("SELECT * FROM mediodepago WHERE codigo=$1", Codigo)
	if err != nil {
		panic(err.Error())
	}
	emp := mediodepago{}
...
func MediodepagoActual(w http.ResponseWriter, r *http.Request) {
	var tabla="mediodepago"
	emp := buscarParametro{}
...
// MEDIO DE PAGO ACTUALIZAR
func MediodepagoActualizar(w http.ResponseWriter, r *http.Request) {
	db := dbConn()
	if r.Method == "POST" {
		codigo := r.FormValue("Codigo")
		nombre := r.FormValue("Nombre")
		nombre = Titulo(nombre)
		insForm, err := db.Prepare("UPDATE mediodepago set	nombre=$2  " + " WHERE codigo=$1")
		if err != nil {
			panic(err.Error())
		}
		insForm.Exec(codigo, nombre)
...
//  DE PAGO BUSCAR
func MediodepagoBuscar(w http.ResponseWriter, r *http.Request) {
	var tabla="mediodepago"
	db := dbConn()
...
// MEDIO DE PAGO BORRAR
func MediodepagoBorrar(w http.ResponseWriter, r *http.Request) {
	tmp, _ := template.ParseFiles("vista/inicio/Modulo.html",
		"vista/mediodepago/mediodepagoBorrar.html")
	db := dbConn()
	Codigo := mux.Vars(r)["codigo"]
	selDB, err := db.Query("SELECT * FROM mediodepago WHERE codigo=$1", Codigo)
	if err != nil {
...
// MEDIO DE PAGO ELIMINAR
func MediodepagoEliminar(w http.ResponseWriter, r *http.Request) {
	log.Println("Inicio Eliminar")
	db := dbConn()
	emp := mux.Vars(r)["codigo"]
	delForm, err := db.Prepare("DELETE from Mediodepago WHERE codigo=$1")
	if err != nil {
		panic(err.Error())
	}
	delForm.Exec(emp)
...
// INICIA  DE PAGO PDF
func MediodepagoPdf(w http.ResponseWriter, r *http.Request) {
	db := dbConn()
	Codigo := mux.Vars(r)["codigo"]
	t := mediodepago{}
	var e  empresa=ListaEmpresa()
	var c  ciudad=TraerCiudad(e.Ciudad)
	err := db.Get(&t, "SELECT * FROM mediodepago where codigo=$1", Codigo)
	if err != nil {
		log.Fatalln(err)
	}
	...
	pdf.SetHeaderFunc(func() {
		...
		pdf.SetX(20)
		pdf.CellFormat(184, 6, " MEDIO DE PAGO", "0", 0,
			"C", true, 0, "")
		pdf.Ln(8)
	})
...
// INICIA MEDIO DE PAGO TODOS PDF
func MediodepagoTodosCabecera(pdf *gofpdf.Fpdf){
	pdf.SetFont("Arial", "", 10)
	// RELLENO TITULO
	pdf.SetY(50)
	pdf.SetFillColor(224,231,239)
...
func MediodepagoTodosDetalle(pdf *gofpdf.Fpdf,miFila mediodepago, a int ){
	ene := pdf.UnicodeTranslatorFromDescriptor("")
	pdf.SetFont("Arial", "", 9)
	pdf.SetX(21)
...
func MediodepagoTodosPdf(w http.ResponseWriter, r *http.Request) {
	db := dbConn()

	t := []mediodepago{}
	var e  empresa=ListaEmpresa()
	var c  ciudad=TraerCiudad(e.Ciudad)
	err := db.Select(&t, "SELECT * FROM mediodepago ORDER BY codigo ASC ")
	if err != nil {
		log.Fatalln(err)
	}
	var buf bytes.Buffer
	...
	for i, miFila := range t {
		...
		MediodepagoTodosDetalle(pdf,miFila,a)
	}
...
// DOCUMENTO EXCEL
func MediodepagoExcel(w http.ResponseWriter, r *http.Request) {
	db := dbConn()
	t := []mediodepago{}
	var e  empresa=ListaEmpresa()
	var c  ciudad=TraerCiudad(e.Ciudad)
