...
type CertificadosalariosParametro struct {
...
type Certificadosalarioslista struct {
	Fila   string
	Codigo string
	Nombre string
	Accion string
}
type cuentaSaldoSalarios struct {
	Cuenta       string
	CuentaNombre string
	Debito       string
	Credito      string
...
func CertificadosalariosDatos(w http.ResponseWriter, r *http.Request) {
	var tempParametro CertificadosalariosParametro
	...
	res := []Certificadosalarioslista{}
	final := []Certificadosalarioslista{}
	...
	for a, miFila := range res {
		...
		final = append(final, Certificadosalarioslista{miFila.Fila, miFila.Codigo, miFila.Nombre, ""})
	}
...
func CertificadosalariosLista(w http.ResponseWriter, r *http.Request) {
	tmp, _ := template.ParseFiles("vista/inicio/Modulo.html",
		"vista/certificadosalarios/CertificadosalariosLista.html",
		"vista/certificadosalarios/Autocompletaplandecuentaempresa.html",
		"vista/certificadosalarios/Autocompletatercero.html")
...
func saldoSalarios(mitercero string, cuentaInicial string, cuentaFinal string, fechaInicial time.Time, fechaFinal time.Time) []cuentaSaldoSalarios {
	...
	listadoSaldoTercero := []cuentaSaldoSalarios{}
	...
	for _, miSaldo := range listadoSaldoTercero {
		log.Println("suma saldo Salarios ")
...
func baseSalarios(mitercero string, fechaInicial time.Time, fechaFinal time.Time) float64 {
	...
	consultasaldoAnterior += " (substring(cuenta,1,4)='5101'  or "
	consultasaldoAnterior += " substring(cuenta,1,4)='5102'  or "
	consultasaldoAnterior += " substring(cuenta,1,4)='5201'  or "
	consultasaldoAnterior += " substring(cuenta,1,4)='5202'  )"
...
func CertificadosalariosPdf(w http.ResponseWriter, r *http.Request) {
	db := dbConn()
	Codigo := mux.Vars(r)["codigo"]
	...
	listadoCuentaSalarios := saldoSalarios(Codigo, CuentaInicial, CuentaFinal, dateinicial, datefinal)
	...
	for _, miCuenta := range listadoCuentaSalarios {

		var nombreCuenta = miCuenta.CuentaNombre
		var valorSalarios = miCuenta.Saldo
		var valorBase = baseSalarios(Codigo, dateinicial, datefinal)
		...
		pdf.SetY(17)
		pdf.SetX(40)
		pdf.CellFormat(140, 4, ene("Certificado de Retenciones Por Rentas de Trabajo y Pensiones"), "0", 0, "C", false, 0, "")
		pdf.SetY(22)
		pdf.SetX(40)
		pdf.CellFormat(140, 4, ene("Articulo 383 del E. T."), "0", 0, "C", false, 0, "")
		pdf.SetY(27)
		pdf.SetX(40)
		...
		pdf.CellFormat(89, 6, "VALOR RETENIDO", "1", 0,
			"L", true, 0, "")
		pdf.CellFormat(91, 6, "$ "+Coma(FormatoFlotanteEntero(Flotante(valorSalarios))), "1", 0,
			"L", true, 0, "")
		pdf.Ln(-1)
		pdf.SetX(21)
...
func CertificadoSalariosTodosDetalle(pdf *gofpdf.Fpdf, miFila Certificadosalarioslista, a int) {
	pdf.SetFont("Arial", "", 9)
	pdf.SetX(21)
...
func CertificadoSalariosTodosCabecera(pdf *gofpdf.Fpdf) {
	pdf.SetFont("Arial", "", 10)
	// RELLENO TITULO
	pdf.SetY(50)
...
func CertificadoSalariosTodosPdf(w http.ResponseWriter, r *http.Request) {
	db := dbConn()
	...
	listaSalarios := []Certificadosalarioslista{}
	...
	err := db.Select(&listaSalarios, consulta, tempParametro.TerceroInicial, tempParametro.TerceroFinal, tempParametro.CuentaInicial, tempParametro.CuentaFinal, tempParametro.FechaInicial, tempParametro.FechaFinal)
	...
	pdf.SetHeaderFunc(func() {
		...
		pdf.CellFormat(190, 10, ene(c.NombreCiudad+" - "+c.NombreDepartamento), "0", 0, "C", false, 0,
			"")
		pdf.Ln(6)
		pdf.CellFormat(190, 10, "Certificado De Salarios", "0", 0,
			"C", false, 0, "")
		pdf.Ln(10)
	})
	...
	CertificadoSalariosTodosCabecera(pdf)
	// tercera pagina

	for i, miFila := range listaSalarios {
		var a = i + 1
		...
		CertificadoSalariosTodosDetalle(pdf, miFila, a)
	}
