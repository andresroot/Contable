...
// ESTRUCTURA FORMA DE PAGO JSON
type formadepagoJson struct {
	Id     string `json:"id"`
...
// FORMA DE PAGO LISTA
func FormadepagoLista(w http.ResponseWriter, r *http.Request) {
	tmp, _ := template.ParseFiles("vista/inicio/Modulo.html",
		"vista/formadepago/formadepagoLista.html")
	db := dbConn()
	selDB, err := db.Query("SELECT * FROM formadepago ORDER BY cast(codigo as integer) ASC")
	if err != nil {
		panic(err.Error())
	}
	res := []formadepago{}
	for selDB.Next() {
		...
		res = append(res, formadepago{Codigo, Nombre })
	}
...
// MEDIO DE PAGO NUEVO
func FormadepagoNuevo(w http.ResponseWriter, r *http.Request) {
	...
	if Codigo == "False"{
		...
		if Elemento == "False"{
			err := db.Get(&emp, "SELECT * FROM formadepago where codigo=$1", Codigo)
			if err != nil {
				log.Fatalln(err)
			}
		}
	...
	"vista/formadepago/formadepagoNuevo.html",
	"vista/formadepago/autocompletaformadepagocrear.html")
...
// FORMA DE PAGO INSERTAR
func FormadepagoInsertar(w http.ResponseWriter, r *http.Request) {
	db := dbConn()
	if r.Method == "POST" {
		Codigo := r.FormValue("Codigo")
		Nombre := r.FormValue("Nombre")
		Nombre = Titulo(Nombre)
		insForm, err := db.Prepare("INSERT INTO formadepago(codigo, nombre)VALUES($1, $2)")
		if err != nil {
			panic(err.Error())
		}
	...
	http.Redirect(w, r, "/FormadepagoLista", 301)
}

// FORMA DE PAGO EXISTE
func FormadepagoExiste(w http.ResponseWriter, r *http.Request) {
	db := dbConn()
	Codigo := mux.Vars(r)["codigo"]
	var total int
	row := db.QueryRow("SELECT COUNT(*) FROM formadepago  WHERE codigo=$1", Codigo)
	err := row.Scan(&total)
	if err != nil {
		log.Fatal(err)
	}
...
// FORMA DE PAGO EDITAR
func FormadepagoEditar(w http.ResponseWriter, r *http.Request) {
	tmp, _ := template.ParseFiles("vista/inicio/Modulo.html",
		"vista/formadepago/formadepagoEditar.html")
	db := dbConn()
	Codigo := mux.Vars(r)["codigo"]
	selDB, err := db.Query("SELECT * FROM formadepago WHERE codigo=$1", Codigo)
	if err != nil {
		panic(err.Error())
	}
	emp := formadepago{}
...
func FormadepagoActual(w http.ResponseWriter, r *http.Request) {
	var tabla="formadepago"
	emp := buscarParametro{}
...
// FORMA DE PAGO ACTUALIZAR
func FormadepagoActualizar(w http.ResponseWriter, r *http.Request) {
	db := dbConn()
	if r.Method == "POST" {
		codigo := r.FormValue("Codigo")
		nombre := r.FormValue("Nombre")
		nombre = Titulo(nombre)
		insForm, err := db.Prepare("UPDATE formadepago set	nombre=$2  " + " WHERE codigo=$1")
		if err != nil {
			panic(err.Error())
		}
		insForm.Exec(codigo, nombre)
...
func FormadepagoBuscar(w http.ResponseWriter, r *http.Request) {
	var tabla="formadepago"
	db := dbConn()
...
// FORMA DE PAGO BORRAR
func FormadepagoBorrar(w http.ResponseWriter, r *http.Request) {
	tmp, _ := template.ParseFiles("vista/inicio/Modulo.html",
		"vista/formadepago/formadepagoBorrar.html")
	db := dbConn()
	Codigo := mux.Vars(r)["codigo"]
	selDB, err := db.Query("SELECT * FROM formadepago WHERE codigo=$1", Codigo)
	if err != nil {
...
// FORMA DE PAGO ELIMINAR
func FormadepagoEliminar(w http.ResponseWriter, r *http.Request) {
	log.Println("Inicio Eliminar")
	db := dbConn()
	emp := mux.Vars(r)["codigo"]
	delForm, err := db.Prepare("DELETE from formadepago WHERE codigo=$1")
	if err != nil {
		panic(err.Error())
	}
	delForm.Exec(emp)
...
// INICIA FORMA DE PAGO PDF
func FormadepagoPdf(w http.ResponseWriter, r *http.Request) {
	db := dbConn()
	Codigo := mux.Vars(r)["codigo"]
	t := formadepago{}
	var e  empresa=ListaEmpresa()
	var c  ciudad=TraerCiudad(e.Ciudad)
	err := db.Get(&t, "SELECT * FROM formadepago where codigo=$1", Codigo)
	if err != nil {
		log.Fatalln(err)
	}
	...
	pdf.SetHeaderFunc(func() {
		...
		pdf.SetX(20)
		pdf.CellFormat(184, 6, "FORMA DE PAGO", "0", 0,
			"C", true, 0, "")
		pdf.Ln(8)
	})
...
// INICIA FORMA DE PAGO TODOS PDF
func FormadepagoTodosCabecera(pdf *gofpdf.Fpdf){
	pdf.SetFont("Arial", "", 10)
	// RELLENO TITULO
	pdf.SetY(50)
	pdf.SetFillColor(224,231,239)
...
func FormadepagoTodosDetalle(pdf *gofpdf.Fpdf,miFila formadepago, a int ){
	ene := pdf.UnicodeTranslatorFromDescriptor("")
	pdf.SetFont("Arial", "", 9)
	pdf.SetX(21)
...
func FormadepagoTodosPdf(w http.ResponseWriter, r *http.Request) {
	db := dbConn()

	t := []formadepago{}
	var e  empresa=ListaEmpresa()
	var c  ciudad=TraerCiudad(e.Ciudad)
	err := db.Select(&t, "SELECT * FROM formadepago ORDER BY cast(codigo as integer) ")
	if err != nil {
		log.Fatalln(err)
	}
	var buf bytes.Buffer
	...
	pdf.SetHeaderFunc(func() {
		...
		pdf.CellFormat(190, 10, ene(c.NombreCiudad+ " - "+c.NombreDepartamento), "0", 0, "C", false, 0,
			"")
		pdf.Ln(6)
		pdf.CellFormat(190, 10, "DATOS FORMA DE PAGO", "0", 0,
			"C", false, 0, "")
		pdf.Ln(10)
	})
	...
	for i, miFila := range t {
		...
		FormadepagoTodosDetalle(pdf,miFila,a)
	}
...
// DOCUMENTO EXCEL
func FormadepagoExcel(w http.ResponseWriter, r *http.Request) {
	db := dbConn()
	t := []formadepago{}
	var e  empresa=ListaEmpresa()
	var c  ciudad=TraerCiudad(e.Ciudad)
	...
	f.SetCellValue("Sheet1", "A9","DATOS FORMA DE PAGO")
