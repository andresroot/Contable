...
// GRUPO ACTUAL
func GrupoActual(w http.ResponseWriter, r *http.Request) {
	var tabla="grupo"
	emp := buscarParametro{}
...
func GrupoNuevo(w http.ResponseWriter, r *http.Request) {
	...
	if Codigo == "False"{
		...
		err := db.Get(&emp, "SELECT * FROM grupo where codigo=$1", Codigo)
		if err != nil {
			log.Fatalln(err)
		}
	}
	...
	tmp, _ := template.ParseFiles("vista/inicio/Modulo.html",
		"vista/grupo/autocompletaGrupocrear.html",
...
func GrupoBuscarCrear(w http.ResponseWriter, r *http.Request) {
	db := dbConn()
	Codigo := mux.Vars(r)["codigo"]
	var tabla="grupo"
...
// CENTRO BUSCAR
func GrupoBuscar(w http.ResponseWriter, r *http.Request) {
	var tabla="grupo"
	db := dbConn()
...
func GrupoPdf(w http.ResponseWriter, r *http.Request) {
	...
	pdf.SetHeaderFunc(func() {
		...
		pdf.SetX(20)
		pdf.CellFormat(184, 6, "DATOS GRUPO", "0", 0,
			"C", true, 0, "")
		pdf.Ln(8)
	})
...
// INICIA GRUPO TODOS PDF
func GrupoTodosCabecera(pdf *gofpdf.Fpdf){
	pdf.SetFont("Arial", "", 10)
	// RELLENO TITULO
	pdf.SetY(50)
	pdf.SetFillColor(224,231,239)
...
func GrupoTodosDetalle(pdf *gofpdf.Fpdf,miFila grupo, a int ){
	pdf.SetFont("Arial", "", 9)
	pdf.SetX(21)
...
func GrupoTodosPdf(w http.ResponseWriter, r *http.Request) {
	db := dbConn()

	t := []grupo{}
	var e  empresa=ListaEmpresa()
	var c  ciudad=TraerCiudad(e.Ciudad)
	err := db.Select(&t, "SELECT * FROM grupo ORDER BY cast(codigo as integer) ")
	if err != nil {
		log.Fatalln(err)
	}
	var buf bytes.Buffer
	...
	for i, miFila := range t {
		...
		GrupoTodosDetalle(pdf,miFila,a)
	}
...
// DOCUMENTO EXCEL
func GrupoExcel(w http.ResponseWriter, r *http.Request) {
	db := dbConn()
	t := []grupo{}
	var e  empresa=ListaEmpresa()
	var c  ciudad=TraerCiudad(e.Ciudad)
	...
	f.SetCellValue("Sheet1", "A9","LISTADO DE GRUPOS")
