...
type Certificademepleadoparametro struct {
...
type Certificadoempleadolista struct {
	Fila   string
	Codigo string
	Nombre string
	Accion string
...
func CertificadoempleadoDatos(w http.ResponseWriter, r *http.Request) {
	//db := dbConn()
	var tempParametro Certificademepleadoparametro
	...
	res := []Certificadoempleadolista{}
	//resfila := []empleadolista{}
	var consulta string

	consulta = " SELECT ROW_NUMBER() OVER(ORDER BY codigo DESC)  as Fila ,codigo,nombre FROM empleado "
	consulta += " WHERE codigo>=$1 and codigo<=$2 "
	consulta += " ORDER BY sueldo DESC "

	db.Select(&res, consulta, tempParametro.TerceroInicial, tempParametro.TerceroFinal)
...
func CertificadoempleadoLista(w http.ResponseWriter, r *http.Request) {
	tmp, _ := template.ParseFiles("vista/inicio/Modulo.html",
		"vista/certificadoempleado/CertificadoempleadoLista.html",
		"vista/certificadoempleado/Autocompletaempleado.html")
	//	db := dbConn()
...
// INICIA CERTIFICADO EMPLEADO 220 PDF
func Certificadoempleado(w http.ResponseWriter, r *http.Request) {
	db := dbConn()
	Codigo := mux.Vars(r)["codigo"]
...
func saldoEmpleado(tercero string, fechaInicial time.Time, fechaFinal time.Time, micuenta string) float64 {
	var consultasaldoAnterior = "SELECT debito,credito FROM comprobantedetalle  where"
	consultasaldoAnterior += " tercero=$1  and (fecha>=$2 and fecha<=$3) and cuenta=$4"
	//
	listadoSaldoTercero := []cuentaSaldo{}
	db.Select(&listadoSaldoTercero, consultasaldoAnterior, tercero, fechaInicial, fechaFinal, micuenta)
