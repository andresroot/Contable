...
func ResolucionventaNuevo(w http.ResponseWriter, r *http.Request) {
	...
	if Codigo == "False"{
		...
		err := db.Get(&emp, "SELECT * FROM resolucionventa where codigo=$1", Codigo)
		if err != nil {
			log.Fatalln(err)
		}
	}
...
func ResolucionventaPdf(w http.ResponseWriter, r *http.Request) {
	...
	pdf.SetHeaderFunc(func() {
		...
		pdf.SetX(20)
		pdf.CellFormat(184, 6, "DATOS RESOLUCION VENTA", "0", 0,
			"C", true, 0, "")
		pdf.Ln(8)
	})
...
// INICIA RESOLUCION VENTA TODOS PDF
func ResolucionventaTodosCabecera(pdf *gofpdf.Fpdf){
	pdf.SetFont("Arial", "", 10)
	// RELLENO TITULO
	pdf.SetY(50)
	pdf.SetFillColor(224,231,239)
...
func ResolucionventaTodosDetalle(pdf *gofpdf.Fpdf,miFila Resolucionventa, a int ){
	pdf.SetFont("Arial", "", 9)
	pdf.SetX(21)
...
func ResolucionventaTodosPdf(w http.ResponseWriter, r *http.Request) {
	db := dbConn()

	t := []Resolucionventa{}
	var e  empresa=ListaEmpresa()
	var c  ciudad=TraerCiudad(e.Ciudad)
	err := db.Select(&t, "SELECT * FROM resolucionventa ORDER BY cast(codigo as integer) ")
	if err != nil {
		log.Fatalln(err)
	}
	var buf bytes.Buffer
	...
	pdf.SetHeaderFunc(func() {
		...
		pdf.CellFormat(190, 10, ene(c.NombreCiudad+ " - "+c.NombreDepartamento), "0", 0, "C", false, 0,
			"")
		pdf.Ln(6)
		pdf.CellFormat(190, 10, "DATOS RESOLUCION VENTA", "0", 0,
			"C", false, 0, "")
		pdf.Ln(10)
	})
	...
	ResolucionventaTodosCabecera(pdf)
	// tercera pagina
	...
	for i, miFila := range t {
		...
		ResolucionventaTodosDetalle(pdf,miFila,a)
	}
...
// TERMINA RESOLUCION VENTA TODOS PDF

// RESOLUCION VENTA EXCEL
func ResolucionventaExcel(w http.ResponseWriter, r *http.Request) {
	db := dbConn()
	t := []Resolucionventa{}
	var e  empresa=ListaEmpresa()
	var c  ciudad=TraerCiudad(e.Ciudad)
	...
	f.SetCellValue("Sheet1", "A9","LISTADO DE RESOLUCION VENTA")
