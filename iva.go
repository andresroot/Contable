...
// IVA LISTA
func IvaLista(w http.ResponseWriter, r *http.Request) {
	tmp, _ := template.ParseFiles("vista/inicio/Modulo.html",
		"vista/iva/ivaLista.html")
	db := dbConn()
	selDB, err := db.Query("SELECT * FROM iva ORDER BY cast(codigo as integer) ASC")
	if err != nil {
		panic(err.Error())
	}
	res := []iva{}
	for selDB.Next() {
		...
		res = append(res, iva{Codigo, Nombre })
	}
...
// IVA NUEVO
func IvaNuevo(w http.ResponseWriter, r *http.Request) {
	...
	if Codigo == "False"{
		...
		if Elemento == "False"{
		err := db.Get(&emp, "SELECT * FROM iva where codigo=$1", Codigo)
		if err != nil {
			log.Fatalln(err)
		}
	}
	...
	tmp, _ := template.ParseFiles("vista/inicio/Modulo.html",
		...
		"vista/iva/autocompletaIvacrear.html")
...
// IVA INSERTAR
func IvaInsertar(w http.ResponseWriter, r *http.Request) {
	db := dbConn()
	if r.Method == "POST" {
		Codigo := r.FormValue("Codigo")
		Nombre := r.FormValue("Nombre")
		Nombre = Titulo(Nombre)
		insForm, err := db.Prepare("INSERT INTO iva(codigo, nombre)VALUES($1, $2)")
		if err != nil {
			panic(err.Error())
		}
	...
	http.Redirect(w, r, "/IvaLista", 301)
}

// IVA EXISTE
func IvaExiste(w http.ResponseWriter, r *http.Request) {
	db := dbConn()
	Codigo := mux.Vars(r)["codigo"]
	var total int
	row := db.QueryRow("SELECT COUNT(*) FROM iva  WHERE codigo=$1", Codigo)
	err := row.Scan(&total)
	if err != nil {
		log.Fatal(err)
	}
...
// IVA EDITAR
func IvaEditar(w http.ResponseWriter, r *http.Request) {
	tmp, _ := template.ParseFiles("vista/inicio/Modulo.html",
		"vista/iva/ivaEditar.html")
	db := dbConn()
	Codigo := mux.Vars(r)["codigo"]
	selDB, err := db.Query("SELECT * FROM iva WHERE codigo=$1", Codigo)
	if err != nil {
		panic(err.Error())
	}
	emp := iva{}
...
// IVA ACTUAL
func IvaActual(w http.ResponseWriter, r *http.Request) {
	var tabla="iva"
	emp := buscarParametro{}
...
func IvaActualConsulta(w http.ResponseWriter, r *http.Request) {
	var tabla="iva"
	Codigo := mux.Vars(r)["codigo"]
	emp := buscarParametro{}
	var res []buscarParametro
	db := dbConn()
	err := db.Get(&emp, "SELECT * FROM "+tabla+" where codigo=$1 ", Codigo)
	switch err {
	case nil:
		log.Printf("tercero found: %+v\n", emp)
		res = append(res, emp)
...
// IVA ACTUALIZAR
func IvaActualizar(w http.ResponseWriter, r *http.Request) {
	db := dbConn()
	if r.Method == "POST" {
		codigo := r.FormValue("Codigo")
		nombre := r.FormValue("Nombre")
		nombre = Titulo(nombre)
		insForm, err := db.Prepare("UPDATE iva set	nombre=$2  " + " WHERE codigo=$1")
		if err != nil {
			panic(err.Error())
		}
		insForm.Exec(codigo, nombre)
...
// IVA BUSCAR NOMBRE
func IvaBuscarCrear(w http.ResponseWriter, r *http.Request) {
	db := dbConn()
	Codigo := mux.Vars(r)["codigo"]
	var tabla="iva"
...
// IVA BUSCAR
func IvaBuscar(w http.ResponseWriter, r *http.Request) {
	var tabla="iva"
	db := dbConn()
...
// IVA BORRAR
func IvaBorrar(w http.ResponseWriter, r *http.Request) {
	tmp, _ := template.ParseFiles("vista/inicio/Modulo.html",
		"vista/iva/ivaBorrar.html")
	db := dbConn()
	Codigo := mux.Vars(r)["codigo"]
	selDB, err := db.Query("SELECT * FROM iva WHERE codigo=$1", Codigo)
	if err != nil {
...
// IVA ELIMINAR
func IvaEliminar(w http.ResponseWriter, r *http.Request) {
	log.Println("Inicio Eliminar")
	db := dbConn()
	emp := mux.Vars(r)["codigo"]
	delForm, err := db.Prepare("DELETE from iva WHERE codigo=$1")
	if err != nil {
		panic(err.Error())
	}
	delForm.Exec(emp)
...
// INICIA IVA PDF
func IvaPdf(w http.ResponseWriter, r *http.Request) {
	db := dbConn()
	Codigo := mux.Vars(r)["codigo"]
	t := iva{}
	var e  empresa=ListaEmpresa()
	var c  ciudad=TraerCiudad(e.Ciudad)
	err := db.Get(&t, "SELECT * FROM iva where codigo=$1", Codigo)
	if err != nil {
		log.Fatalln(err)
	}
	...
	pdf.SetHeaderFunc(func() {
		...
		pdf.SetX(20)
		pdf.CellFormat(184, 6, "IVA DE COSTOS", "0", 0,
			"C", true, 0, "")
		pdf.Ln(8)
	})
...
// INICIA IVA TODOS PDF
func IvaTodosCabecera(pdf *gofpdf.Fpdf){
	pdf.SetFont("Arial", "", 10)
	// RELLENO TITULO
	pdf.SetY(50)
	pdf.SetFillColor(224,231,239)
...
func IvaTodosDetalle(pdf *gofpdf.Fpdf,miFila iva, a int ){
	pdf.SetFont("Arial", "", 9)
	pdf.SetX(21)
...
func IvaTodosPdf(w http.ResponseWriter, r *http.Request) {
	db := dbConn()

	t := []iva{}
	var e  empresa=ListaEmpresa()
	var c  ciudad=TraerCiudad(e.Ciudad)
	err := db.Select(&t, "SELECT * FROM iva ORDER BY cast(codigo as integer) ")
	if err != nil {
		log.Fatalln(err)
	}
	var buf bytes.Buffer
	...
	pdf.SetHeaderFunc(func() {
		...
		pdf.CellFormat(190, 10, ene(c.NombreCiudad+ " - "+c.NombreDepartamento), "0", 0, "C", false, 0,
			"")
		pdf.Ln(6)
		pdf.CellFormat(190, 10, "DATOS IVA DE COSTOS", "0", 0,
			"C", false, 0, "")
		pdf.Ln(10)
	})
...
// DOCUMENTO EXCEL
func IvaExcel(w http.ResponseWriter, r *http.Request) {
	db := dbConn()
	t := []iva{}
	var e  empresa=ListaEmpresa()
	var c  ciudad=TraerCiudad(e.Ciudad)
	...
	f.SetCellValue("Sheet1", "A9","DATOS IVA DE COSTOS")
