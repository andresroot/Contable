...
func ResolucionsoporteNuevo(w http.ResponseWriter, r *http.Request) {
	...
	emp := Resolucionsoporte{}
	if Codigo == "False"{
	} else {
		err := db.Get(&emp, "SELECT * FROM resolucionsoporte where codigo=$1", Codigo)
		if err != nil {
			log.Fatalln(err)
		}
	}
...
func ResolucionsoportePdf(w http.ResponseWriter, r *http.Request) {
	...
	pdf.SetHeaderFunc(func() {
		...
		pdf.SetX(20)
		pdf.CellFormat(184, 6, "DATOS RESOLUCION SOPORTE", "0", 0,
			"C", true, 0, "")
		pdf.Ln(8)
	})
...
// INICIA RESOLUCION SOPORTE TODOS PDF
func ResolucionsoporteTodosCabecera(pdf *gofpdf.Fpdf){
	pdf.SetFont("Arial", "", 10)
	// RELLENO TITULO
	pdf.SetY(50)
	pdf.SetFillColor(224,231,239)
...
func ResolucionsoporteTodosDetalle(pdf *gofpdf.Fpdf,miFila Resolucionsoporte, a int ){
	pdf.SetFont("Arial", "", 9)
	pdf.SetX(21)
...
func ResolucionsoporteTodosPdf(w http.ResponseWriter, r *http.Request) {
	db := dbConn()

	t := []Resolucionsoporte{}
	var e  empresa=ListaEmpresa()
	var c  ciudad=TraerCiudad(e.Ciudad)
	err := db.Select(&t, "SELECT * FROM resolucionsoporte ORDER BY cast(codigo as integer) ")
	if err != nil {
		log.Fatalln(err)
	}
	var buf bytes.Buffer
	...
	pdf.SetHeaderFunc(func() {
		...
		pdf.CellFormat(190, 10, ene(c.NombreCiudad+ " - "+c.NombreDepartamento), "0", 0, "C", false, 0,
			"")
		pdf.Ln(6)
		pdf.CellFormat(190, 10, "DATOS RESOLUCION SOPORTE", "0", 0,
			"C", false, 0, "")
		pdf.Ln(10)
	})
	...
	ResolucionsoporteTodosCabecera(pdf)
	// tercera pagina
	...
	for i, miFila := range t {
		...
		ResolucionsoporteTodosDetalle(pdf,miFila,a)
	}
...
// TERMINA RESOLUCION SOPORTE TODOS PDF

// RESOLUCION SOPORTE EXCEL
func ResolucionsoporteExcel(w http.ResponseWriter, r *http.Request) {
	db := dbConn()
	t := []Resolucionsoporte{}
	var e  empresa=ListaEmpresa()
	var c  ciudad=TraerCiudad(e.Ciudad)
	...
	f.SetCellValue("Sheet1", "A9","LISTADO DE RESOLUCION SOPORTE")
