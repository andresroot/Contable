...
func FinancieroLista(w http.ResponseWriter, r *http.Request) {
	...
	db := dbConn()
	selDB, err := db.Query("SELECT * FROM financiero ORDER BY cast(codigo as integer) DESC")
	if err != nil {
		panic(err.Error())
	}
	res := []financiero{}
...
func FinancieroTodosPdf(w http.ResponseWriter, r *http.Request) {
	...
	var e  empresa=ListaEmpresa()
	var c  ciudad=TraerCiudad(e.Ciudad)
	err := db.Select(&t, "SELECT * FROM financiero ORDER BY cast(codigo as integer) DESC")
	if err != nil {
		log.Fatalln(err)
	}
	var buf bytes.Buffer
...
func FinancieroExcel(w http.ResponseWriter, r *http.Request) {
	...
	f.SetCellValue("Sheet1", "A9","LISTADO DE FINANCIEROS")
