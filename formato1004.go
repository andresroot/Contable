...
func Formato1004(w http.ResponseWriter, r *http.Request) {
	listaformato := []miFormatoExcel{}
	listaformato = generarformato("1004", true)
	f := excelize.NewFile()
	...
	f.SetCellValue("Sheet1", "M1", "CORREO")
	f.SetCellValue("Sheet1", "N1", "COLUMNA1")
	f.SetCellValue("Sheet1", "O1", "COLUMNA2")
	...
	for i, miFila := range listaformato {
		...
		f.SetCellValue("Sheet1", "M"+a, miFila.Correo)
		f.SetCellValue("Sheet1", "N"+a, Flotante(miFila.Columna1))
		f.SetCellValue("Sheet1", "O"+a, Flotante(miFila.Columna2))
