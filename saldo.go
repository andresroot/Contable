...
// CENTRO TABLA
type saldo struct {
	NombreBodega    string  `json:"NombreBodega"`
	NombreProducto  string  `json:"NombreProducto"`
...
func SaldoDatosProductoBodega(w http.ResponseWriter, r *http.Request) {
	db := dbConn()
	Codigo := mux.Vars(r)["codigo"]
	Bodega := mux.Vars(r)["bodega"]
	var q string
	q="  select bodega,'' as nombrebodega,saldo.cantidad from saldo "
	q+=" where saldo.producto=$1 and saldo.bodega=$2"
	saldofinal:= []saldoVenta{}
	err2 := db.Select(&saldofinal, q,Codigo,Bodega)
	if err2 != nil {
		fmt.Println(err2)
	}
	if len(saldofinal)==0 {
...
func SaldoDatosProducto(w http.ResponseWriter, r *http.Request) {
	db := dbConn()
	Codigo := mux.Vars(r)["codigo"]
	var q string
	q="  select bodega.nombre as nombrebodega,bodega,saldo.cantidad from saldo "
	q+=" inner join bodega on bodega.codigo=saldo.bodega"
	q+=" where saldo.producto=$1 order by producto,bodega"
	saldofinal:= []saldoVenta{}
	err2 := db.Select(&saldofinal, q,Codigo)
	if err2 != nil {
		fmt.Println(err2)
	}
	if len(saldofinal)==0 {
...
func SaldoDatosTodosNueva(w http.ResponseWriter, r *http.Request) {
	var q string
	...
	q=" 	select '' as filas ,producto,producto.nombre as NombreProducto,producto.costo "
	q+=" 	,sum(saldo.cantidad)  as cantidad "
	q+=" 		,sum(saldo.cantidad)*producto.costo  as total from saldo "
	q+=" 	inner join producto on producto.codigo=saldo.producto group by producto.nombre,saldo.producto,producto.costo order by saldo.producto "

	saldofinal:= []saldoCosto{}
	...
	var total float64
	total=0
	for _, x := range saldofinal {
		total+=x.Total
	}

	saldofinal=append(saldofinal,saldoCosto{"","","Total",0,0,total})
...
func SaldoDatosTodos(w http.ResponseWriter, r *http.Request) {

	var 	ProductoParametro string
	Discriminar := "Si"
	...
	db.Select(&listaProducto, "SELECT * FROM producto ORDER BY codigo ASC")

	saldofinal:= []saldoCosto{}

	var total float64

	for _ , miProducto := range listaProducto {

		ProductoParametro=miProducto.Codigo
		...
		var micantidad float64
		var micosto float64
		var mitotal float64

		for _ , miSaldo := range listadokardexfinal {
			 micantidad=miSaldo.Cantidadsaldo
			 micosto =miSaldo.Preciosaldo
			 mitotal=miSaldo.Totalsaldo
		}


		saldofinal=append(saldofinal,saldoCosto{"",miProducto.Codigo,miProducto.Nombre,micantidad,micosto,mitotal})
		total+=mitotal

	}

	saldofinal=append(saldofinal,saldoCosto{"","","",0,0,total})
...
// CENTRO KARDEX
func SaldoLista(w http.ResponseWriter, r *http.Request) {
	tmp, _ := template.ParseFiles("vista/inicio/Modulo.html",
		"vista/saldo/saldoLista.html")
	//	db := dbConn()
	Codigo := mux.Vars(r)["codigo"]
	//res := []inventario{}
...
func SaldoDatosCabecera(pdf *gofpdf.Fpdf ){
...
func SaldoDatosFilaDetalle(pdf *gofpdf.Fpdf,miFila kardex, a int ){
	pdf.SetFont("Arial", "", 8)
	pdf.SetTextColor(0,0,0)
...
func SaldoDatosPdf(w http.ResponseWriter, r *http.Request) {
...
// KARDEX EXCEL
func SaldoDatosExcel(w http.ResponseWriter, r *http.Request) {
	ProductoParametro := mux.Vars(r)["codigo"]
	Discriminar := mux.Vars(r)["discriminar"]
	FechaInicial := mux.Vars(r)["fechainicial"]
	FechaFinal := mux.Vars(r)["fechafinal"]
