...
// ESTRUCTURA RESPONSABILIDAD FISCAL JSON
type responsabilidadfiscalJson struct {
	Id     string `json:"id"`
...
// ESTRUCTURA RESPONSABILIDAD FISCAL
type responsabilidadfiscal struct {
	Codigo      string
	Nombre      string
}

// RESPONSABILIDAD FISCAL LISTA
func ResponsabilidadfiscalLista(w http.ResponseWriter, r *http.Request) {
	tmp, _ := template.ParseFiles("vista/inicio/Modulo.html",
		"vista/responsabilidadfiscal/responsabilidadfiscalLista.html")
	db := dbConn()
	selDB, err := db.Query("SELECT * FROM responsabilidadfiscal ORDER BY codigo ASC")
	if err != nil {
		panic(err.Error())
	}
	res := []responsabilidadfiscal{}
	for selDB.Next() {
		var Codigo string
		var Nombre string
		...
		res = append(res, responsabilidadfiscal{Codigo, Nombre })
	}
...
// RESPONSABILIDAD FISCAL NUEVO
func ResponsabilidadfiscalNuevo(w http.ResponseWriter, r *http.Request) {
	// TRAER COPIA DE EDITAR
	db := dbConn()
	Codigo := mux.Vars(r)["codigo"]
	Panel := mux.Vars(r)["panel"]
	Elemento := mux.Vars(r)["elemento"]
	emp := responsabilidadfiscal{}
	if Codigo == "False"{
	} else {
		if Elemento == "False"{
			err := db.Get(&emp, "SELECT * FROM responsabilidadfiscal where codigo=$1", Codigo)
			if err != nil {
				log.Fatalln(err)
			}
		}
	...
	miTemplate, err := template.ParseFiles("vista/inicio/Modulo.html",
		"vista/responsabilidadfiscal/responsabilidadfiscalNuevo.html",
		"vista/responsabilidadfiscal/autocompletaresponsabilidadfiscalcrear.html")
...
// RESPONSABILIDAD FISCAL INSERTAR
func ResponsabilidadfiscalInsertar(w http.ResponseWriter, r *http.Request) {
	db := dbConn()
	if r.Method == "POST" {
		Codigo := r.FormValue("Codigo")
		Nombre := r.FormValue("Nombre")
		Nombre = Titulo(Nombre)
		insForm, err := db.Prepare("INSERT INTO responsabilidadfiscal(codigo, nombre)VALUES($1, $2)")
		if err != nil {
		panic(err.Error())
		}
	...
	http.Redirect(w, r, "/ResponsabilidadfiscalLista", 301)
}

// 	RESPONSABILIDAD FISCAL EXISTE
func ResponsabilidadfiscalExiste(w http.ResponseWriter, r *http.Request) {
	db := dbConn()
	Codigo := mux.Vars(r)["codigo"]
	var total int
	row := db.QueryRow("SELECT COUNT(*) FROM responsabilidadfiscal  WHERE codigo=$1", Codigo)
	err := row.Scan(&total)
	if err != nil {
		log.Fatal(err)
	}
...
// RESPONSABILIDAD FISCAL EDITAR
func ResponsabilidadfiscalEditar(w http.ResponseWriter, r *http.Request) {
	tmp, _ := template.ParseFiles("vista/inicio/Modulo.html",
		"vista/responsabilidadfiscal/responsabilidadfiscalEditar.html")
	db := dbConn()
	Codigo := mux.Vars(r)["codigo"]
	selDB, err := db.Query("SELECT * FROM responsabilidadfiscal WHERE codigo=$1", Codigo)
	if err != nil {
		panic(err.Error())
	}
	emp := responsabilidadfiscal{}
...
// RESPONSABILIDAD FISCAL ACTUAL
func ResponsabilidadfiscalActual(w http.ResponseWriter, r *http.Request) {
	var tabla="responsabilidadfiscal"
	emp := buscarParametro{}
...
// RESPONSABILIDAD FISCAL ACTUALIZAR
func ResponsabilidadfiscalActualizar(w http.ResponseWriter, r *http.Request) {
	db := dbConn()
	if r.Method == "POST" {
		codigo := r.FormValue("Codigo")
		nombre := r.FormValue("Nombre")
		nombre = Titulo(nombre)
		insForm, err := db.Prepare("UPDATE responsabilidadfiscal set	nombre=$2  " + " WHERE codigo=$1")
		if err != nil {
			panic(err.Error())
		}
		insForm.Exec(codigo, nombre)
...
//  DE PAGO BUSCAR
func ResponsabilidadfiscalBuscar(w http.ResponseWriter, r *http.Request) {
	var tabla="responsabilidadfiscal"
	db := dbConn()
...
// RESPONSABILIDAD FISCAL BORRAR
func ResponsabilidadfiscalBorrar(w http.ResponseWriter, r *http.Request) {
	tmp, _ := template.ParseFiles("vista/inicio/Modulo.html",
		"vista/responsabilidadfiscal/responsabilidadfiscalBorrar.html")
	db := dbConn()
	Codigo := mux.Vars(r)["codigo"]
	selDB, err := db.Query("SELECT * FROM responsabilidadfiscal WHERE codigo=$1", Codigo)
	if err != nil {
...
// RESPONSABILIDAD FISCAL ELIMINAR
func ResponsabilidadfiscalEliminar(w http.ResponseWriter, r *http.Request) {
	log.Println("Inicio Eliminar")
	db := dbConn()
	emp := mux.Vars(r)["codigo"]
	delForm, err := db.Prepare("DELETE from Responsabilidadfiscal WHERE codigo=$1")
	if err != nil {
		panic(err.Error())
	}
	delForm.Exec(emp)
...
// INICIA  DE PAGO PDF
func ResponsabilidadfiscalPdf(w http.ResponseWriter, r *http.Request) {
	db := dbConn()
	Codigo := mux.Vars(r)["codigo"]
	t := responsabilidadfiscal{}
	var e  empresa=ListaEmpresa()
	var c  ciudad=TraerCiudad(e.Ciudad)
	err := db.Get(&t, "SELECT * FROM responsabilidadfiscal where codigo=$1", Codigo)
	if err != nil {
		log.Fatalln(err)
	}
	...
	pdf.SetHeaderFunc(func() {
		...
		pdf.SetX(20)
		pdf.CellFormat(184, 6, " RESPONSABILIDAD FISCAL", "0", 0,
			"C", true, 0, "")
		pdf.Ln(8)
	})
...
// INICIA RESPONSABILIDAD FISCAL TODOS PDF
func ResponsabilidadfiscalTodosCabecera(pdf *gofpdf.Fpdf){
	pdf.SetFont("Arial", "", 10)
	// RELLENO TITULO
	pdf.SetY(50)
	pdf.SetFillColor(224,231,239)
...
func ResponsabilidadfiscalTodosDetalle(pdf *gofpdf.Fpdf,miFila responsabilidadfiscal, a int ){
	ene := pdf.UnicodeTranslatorFromDescriptor("")
	pdf.SetFont("Arial", "", 9)
	pdf.SetX(21)
...
func ResponsabilidadfiscalTodosPdf(w http.ResponseWriter, r *http.Request) {
	db := dbConn()

	t := []responsabilidadfiscal{}
	var e  empresa=ListaEmpresa()
	var c  ciudad=TraerCiudad(e.Ciudad)
	err := db.Select(&t, "SELECT * FROM responsabilidadfiscal ORDER BY codigo ASC ")
	if err != nil {
		log.Fatalln(err)
	}
	var buf bytes.Buffer
	...
	pdf.SetHeaderFunc(func() {
		...
		pdf.CellFormat(190, 10, ene(c.NombreCiudad+ " - "+c.NombreDepartamento), "0", 0, "C", false, 0,
			"")
		pdf.Ln(6)
		pdf.CellFormat(190, 10, "DATOS  DE PAGO", "0", 0,
			"C", false, 0, "")
		pdf.Ln(10)
	})
	...
	ResponsabilidadfiscalTodosCabecera(pdf)
	// tercera pagina
	...
	for i, miFila := range t {
		...
		ResponsabilidadfiscalTodosDetalle(pdf,miFila,a)
	}
...
// DOCUMENTO EXCEL
func ResponsabilidadfiscalExcel(w http.ResponseWriter, r *http.Request) {
	db := dbConn()
	t := []responsabilidadfiscal{}
	var e  empresa=ListaEmpresa()
	var c  ciudad=TraerCiudad(e.Ciudad)
	...
	f.SetCellValue("Sheet1", "A9","DATOS  DE PAGO")
