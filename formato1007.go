...
func Formato1007(w http.ResponseWriter, r *http.Request) {
	listaformato := []miFormatoExcel{}
	listaformato = generarformato("1007", false)
	f := excelize.NewFile()
	...
	f.SetCellValue("Sheet1", "I1", "PAIS")
	f.SetCellValue("Sheet1", "J1", "COLUMNA1")
	f.SetCellValue("Sheet1", "K1", "COLUMNA2")
	var filaExcel = 2
	//var a string
	//	var a = ""
	//var van int
	...
	for i, miFila := range listaformato {
		...
		f.SetCellValue("Sheet1", "I"+a, miFila.Pais)
		f.SetCellValue("Sheet1", "J"+a, Flotante(miFila.Columna1))
		f.SetCellValue("Sheet1", "K"+a, Flotante(miFila.Columna2))
