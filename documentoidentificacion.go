...
// ESTRUCTURA DOCUMENTO IDENTIFICACION JSON
type documentoidentificacionJson struct {
	Id     string `json:"id"`
...
// ESTRUCTURA DOCUMENTO IDENTIFICACION
type documentoidentificacion struct {
	Codigo      string
	Nombre      string
}

// DOCUMENTO IDENTIFICACION LISTA
func DocumentoidentificacionLista(w http.ResponseWriter, r *http.Request) {
	tmp, _ := template.ParseFiles("vista/inicio/Modulo.html",
		"vista/documentoidentificacion/documentoidentificacionLista.html")
	db := dbConn()
	selDB, err := db.Query("SELECT * FROM documentoidentificacion ORDER BY cast(codigo as integer) ASC")
	if err != nil {
		panic(err.Error())
	}
	res := []documentoidentificacion{}
	for selDB.Next() {
		var Codigo string
		var Nombre string
		...
		res = append(res, documentoidentificacion{Codigo, Nombre })
	}
...
// MEDIO DE PAGO NUEVO
func DocumentoidentificacionNuevo(w http.ResponseWriter, r *http.Request) {
	...
	if Codigo == "False"{
		...
		if Elemento == "False"{
			err := db.Get(&emp, "SELECT * FROM documentoidentificacion where codigo=$1", Codigo)
			if err != nil {
				log.Fatalln(err)
			}
		}
	...
	"vista/documentoidentificacion/documentoidentificacionNuevo.html",
	"vista/documentoidentificacion/autocompletadocumentoidentificacioncrear.html")
...
// DOCUMENTO IDENTIFICACION INSERTAR
func DocumentoidentificacionInsertar(w http.ResponseWriter, r *http.Request) {
	db := dbConn()
	if r.Method == "POST" {
		Codigo := r.FormValue("Codigo")
		Nombre := r.FormValue("Nombre")
		Nombre = Titulo(Nombre)
		insForm, err := db.Prepare("INSERT INTO documentoidentificacion(codigo, nombre)VALUES($1, $2)")
		if err != nil {
			panic(err.Error())
		}
	...
	http.Redirect(w, r, "/DocumentoidentificacionLista", 301)
}

// DOCUMENTO IDENTIFICACION EXISTE
func DocumentoidentificacionExiste(w http.ResponseWriter, r *http.Request) {
	db := dbConn()
	Codigo := mux.Vars(r)["codigo"]
	var total int
	row := db.QueryRow("SELECT COUNT(*) FROM documentoidentificacion  WHERE codigo=$1", Codigo)
	err := row.Scan(&total)
	if err != nil {
		log.Fatal(err)
	}
...
// DOCUMENTO IDENTIFICACION EDITAR
func DocumentoidentificacionEditar(w http.ResponseWriter, r *http.Request) {
	tmp, _ := template.ParseFiles("vista/inicio/Modulo.html",
		"vista/documentoidentificacion/documentoidentificacionEditar.html")
	db := dbConn()
	Codigo := mux.Vars(r)["codigo"]
	selDB, err := db.Query("SELECT * FROM documentoidentificacion WHERE codigo=$1", Codigo)
	if err != nil {
		panic(err.Error())
	}
	emp := documentoidentificacion{}
	for selDB.Next() {
		var codigo string
		var nombre string
		err = selDB.Scan(&codigo, &nombre)
...
// DOCUMENTO IDENTIFICACION ACTUAL
func DocumentoidentificacionActual(w http.ResponseWriter, r *http.Request) {
	var tabla="documentoidentificacion"
	emp := buscarParametro{}
...
// DOCUMENTO IDENTIFICACION ACTUALIZAR
func DocumentoidentificacionActualizar(w http.ResponseWriter, r *http.Request) {
	db := dbConn()
	if r.Method == "POST" {
		codigo := r.FormValue("Codigo")
		nombre := r.FormValue("Nombre")
		nombre = Titulo(nombre)
		insForm, err := db.Prepare("UPDATE documentoidentificacion set	nombre=$2  " + " WHERE codigo=$1")
		if err != nil {
			panic(err.Error())
		}
		insForm.Exec(codigo, nombre)
...
// MEDIO DE PAGO BUSCAR
func DocumentoidentificacionBuscar(w http.ResponseWriter, r *http.Request) {
	var tabla="documentoidentificacion"
	db := dbConn()
...
// DOCUMENTO IDENTIFICACION BORRAR
func DocumentoidentificacionBorrar(w http.ResponseWriter, r *http.Request) {
	tmp, _ := template.ParseFiles("vista/inicio/Modulo.html",
		"vista/documentoidentificacion/documentoidentificacionBorrar.html")
	db := dbConn()
	Codigo := mux.Vars(r)["codigo"]
	selDB, err := db.Query("SELECT * FROM documentoidentificacion WHERE codigo=$1", Codigo)
	if err != nil {
...
// DOCUMENTO IDENTIFICACION ELIMINAR
func DocumentoidentificacionEliminar(w http.ResponseWriter, r *http.Request) {
	log.Println("Inicio Eliminar")
	db := dbConn()
	emp := mux.Vars(r)["codigo"]
	delForm, err := db.Prepare("DELETE from documentoidentificacion WHERE codigo=$1")
	if err != nil {
		panic(err.Error())
	}
	delForm.Exec(emp)
...
// INICIA DOCUMENTO IDENTIFICACION PDF
func DocumentoidentificacionPdf(w http.ResponseWriter, r *http.Request) {
	db := dbConn()
	Codigo := mux.Vars(r)["codigo"]
	t := documentoidentificacion{}
	var e  empresa=ListaEmpresa()
	var c  ciudad=TraerCiudad(e.Ciudad)
	err := db.Get(&t, "SELECT * FROM documentoidentificacion where codigo=$1", Codigo)
	if err != nil {
		log.Fatalln(err)
	}
	...
	pdf.SetHeaderFunc(func() {
		...
		pdf.SetX(20)
		pdf.CellFormat(184, 6, "DOCUMENTO IDENTIFICACION", "0", 0,
			"C", true, 0, "")
		pdf.Ln(8)
	})
	...
	pdf.CellFormat(40, 4, "Nombre:", "", 0,
		"", false, 0, "")
	pdf.CellFormat(40, 4, ene(t.Nombre), "", 0,
		"", false, 0, "")
...
// INICIA DOCUMENTO IDENTIFICACION TODOS PDF
func DocumentoidentificacionTodosCabecera(pdf *gofpdf.Fpdf){
	pdf.SetFont("Arial", "", 10)
	// RELLENO TITULO
	pdf.SetY(50)
	pdf.SetFillColor(224,231,239)
...
func DocumentoidentificacionTodosDetalle(pdf *gofpdf.Fpdf,miFila documentoidentificacion, a int ){
	ene := pdf.UnicodeTranslatorFromDescriptor("")
	pdf.SetFont("Arial", "", 9)
	pdf.SetX(21)
	...
	pdf.CellFormat(40, 4, Subcadena(miFila.Codigo,0,12), "", 0,
		"L", false, 0, "")
	pdf.SetX(55)
	pdf.CellFormat(40, 4, ene(miFila.Nombre), "", 0,"L", false, 0, "")

	pdf.Ln(4)
}

func DocumentoidentificacionTodosPdf(w http.ResponseWriter, r *http.Request) {
	db := dbConn()

	t := []documentoidentificacion{}
	var e  empresa=ListaEmpresa()
	var c  ciudad=TraerCiudad(e.Ciudad)
	err := db.Select(&t, "SELECT * FROM documentoidentificacion ORDER BY cast(codigo as integer) ")
	if err != nil {
		log.Fatalln(err)
	}
	var buf bytes.Buffer
	...
	pdf.SetHeaderFunc(func() {
		...
		pdf.CellFormat(190, 10, ene(c.NombreCiudad+ " - "+c.NombreDepartamento), "0", 0, "C", false, 0,
			"")
		pdf.Ln(6)
		pdf.CellFormat(190, 10, "DATOS DOCUMENTO IDENTIFICACION", "0", 0,
			"C", false, 0, "")
		pdf.Ln(10)
	})
	...
	DocumentoidentificacionTodosCabecera(pdf)
	// tercera pagina
	...
	for i, miFila := range t {
		...
		DocumentoidentificacionTodosDetalle(pdf,miFila,a)
	}
...
// DOCUMENTO EXCEL
func DocumentoidentificacionExcel(w http.ResponseWriter, r *http.Request) {
	db := dbConn()
	t := []documentoidentificacion{}
	var e  empresa=ListaEmpresa()
	var c  ciudad=TraerCiudad(e.Ciudad)
	...
	f.SetCellValue("Sheet1", "A9","DATOS DOCUMENTO IDENTIFICACION")
