...
func TerceroNuevo(w http.ResponseWriter, r *http.Request) {
	...
	tmp, _ := template.ParseFiles("vista/inicio/Modulo.html",
		...
		"vista/autocompleta/autocompletaTerceroph.html",
...
func TerceroActualConsulta(w http.ResponseWriter, r *http.Request) {
	var tabla = "tercero"
	emp := buscarParametro{}
	...
	err = db.Get(&emp, "SELECT codigo, nombre FROM "+tabla+" where  codigo=$2 or codigo=$1 or UPPER(nombre)=$2", tempParametro.Codigo, Mayuscula(tempParametro.Nombre))
	switch err {
	case nil:
...
func TerceroTodosDetalle(pdf *gofpdf.Fpdf, t tercero, a int) {
	...
	pdf.CellFormat(40, 4, Coma(t.Codigo)+" - "+t.Dv, "", 0,
		"L", false, 0, "")
	pdf.SetX(60)
	pdf.CellFormat(40, 4, ene(t.Nombre), "", 0, "L", false, 0, "")
	pdf.SetX(120)
	pdf.CellFormat(40, 4, t.Direccion, "", 0,
		"L", false, 0, "")
	pdf.SetX(155)
...
func TerceroTodosPdf(w http.ResponseWriter, r *http.Request) {
	...
	for i, miFila := range t {
		TerceroTodosDetalle(pdf, miFila, i+1)
