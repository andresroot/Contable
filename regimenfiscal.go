...
// ESTRUCTURA REGIMEN FISCAL JSON
type regimenfiscalJson struct {
	Id     string `json:"id"`
...
// ESTRUCTURA REGIMEN FISCAL
type regimenfiscal struct {
	Codigo      string
	Nombre      string
}

// REGIMEN FISCAL LISTA
func RegimenfiscalLista(w http.ResponseWriter, r *http.Request) {
	tmp, _ := template.ParseFiles("vista/inicio/Modulo.html",
		"vista/regimenfiscal/regimenfiscalLista.html")
	db := dbConn()
	selDB, err := db.Query("SELECT * FROM regimenfiscal ORDER BY cast(codigo as integer) ASC")
	if err != nil {
		panic(err.Error())
	}
	res := []regimenfiscal{}
	for selDB.Next() {
		...
		res = append(res, regimenfiscal{Codigo, Nombre })
	}
...
// MEDIO DE PAGO NUEVO
func RegimenfiscalNuevo(w http.ResponseWriter, r *http.Request) {
	...
	if Codigo == "False"{
		...
		if Elemento == "False"{
			err := db.Get(&emp, "SELECT * FROM regimenfiscal where codigo=$1", Codigo)
			if err != nil {
				log.Fatalln(err)
			}
		}
	...
	"vista/regimenfiscal/regimenfiscalNuevo.html",
	"vista/regimenfiscal/autocompletaregimenfiscalcrear.html")
...
// REGIMEN FISCAL INSERTAR
func RegimenfiscalInsertar(w http.ResponseWriter, r *http.Request) {
	db := dbConn()
	if r.Method == "POST" {
		Codigo := r.FormValue("Codigo")
		Nombre := r.FormValue("Nombre")
		Nombre = Titulo(Nombre)
		insForm, err := db.Prepare("INSERT INTO regimenfiscal(codigo, nombre)VALUES($1, $2)")
		if err != nil {
			panic(err.Error())
		}
	...
	http.Redirect(w, r, "/RegimenfiscalLista", 301)
}

// REGIMEN FISCAL EXISTE
func RegimenfiscalExiste(w http.ResponseWriter, r *http.Request) {
	db := dbConn()
	Codigo := mux.Vars(r)["codigo"]
	var total int
	row := db.QueryRow("SELECT COUNT(*) FROM regimenfiscal  WHERE codigo=$1", Codigo)
	err := row.Scan(&total)
	if err != nil {
		log.Fatal(err)
	}
...
// REGIMEN FISCAL EDITAR
func RegimenfiscalEditar(w http.ResponseWriter, r *http.Request) {
	tmp, _ := template.ParseFiles("vista/inicio/Modulo.html",
		"vista/regimenfiscal/regimenfiscalEditar.html")
	db := dbConn()
	Codigo := mux.Vars(r)["codigo"]
	selDB, err := db.Query("SELECT * FROM regimenfiscal WHERE codigo=$1", Codigo)
	if err != nil {
		panic(err.Error())
	}
	emp := regimenfiscal{}
...
// REGIMEN FISCAL ACTUAL
func RegimenfiscalActual(w http.ResponseWriter, r *http.Request) {
	var tabla="regimenfiscal"
	emp := buscarParametro{}
...
// REGIMEN FISCAL ACTUALIZAR
func RegimenfiscalActualizar(w http.ResponseWriter, r *http.Request) {
	db := dbConn()
	if r.Method == "POST" {
		codigo := r.FormValue("Codigo")
		nombre := r.FormValue("Nombre")
		nombre = Titulo(nombre)
		insForm, err := db.Prepare("UPDATE regimenfiscal set	nombre=$2  " + " WHERE codigo=$1")
		if err != nil {
			panic(err.Error())
		}
		insForm.Exec(codigo, nombre)
...
func RegimenfiscalBuscar(w http.ResponseWriter, r *http.Request) {
	var tabla="regimenfiscal"
	db := dbConn()
...
// REGIMEN FISCAL BORRAR
func RegimenfiscalBorrar(w http.ResponseWriter, r *http.Request) {
	tmp, _ := template.ParseFiles("vista/inicio/Modulo.html",
		"vista/regimenfiscal/regimenfiscalBorrar.html")
	db := dbConn()
	Codigo := mux.Vars(r)["codigo"]
	selDB, err := db.Query("SELECT * FROM regimenfiscal WHERE codigo=$1", Codigo)
	if err != nil {
...
// REGIMEN FISCAL ELIMINAR
func RegimenfiscalEliminar(w http.ResponseWriter, r *http.Request) {
	log.Println("Inicio Eliminar")
	db := dbConn()
	emp := mux.Vars(r)["codigo"]
	delForm, err := db.Prepare("DELETE from regimenfiscal WHERE codigo=$1")
	if err != nil {
		panic(err.Error())
	}
	delForm.Exec(emp)
...
// INICIA REGIMEN FISCAL PDF
func RegimenfiscalPdf(w http.ResponseWriter, r *http.Request) {
	db := dbConn()
	Codigo := mux.Vars(r)["codigo"]
	t := regimenfiscal{}
	var e  empresa=ListaEmpresa()
	var c  ciudad=TraerCiudad(e.Ciudad)
	err := db.Get(&t, "SELECT * FROM regimenfiscal where codigo=$1", Codigo)
	if err != nil {
		log.Fatalln(err)
	}
	...
	pdf.SetHeaderFunc(func() {
		...
		pdf.SetX(20)
		pdf.CellFormat(184, 6, "REGIMEN FISCAL", "0", 0,
			"C", true, 0, "")
		pdf.Ln(8)
	})
...
// INICIA REGIMEN FISCAL TODOS PDF
func RegimenfiscalTodosCabecera(pdf *gofpdf.Fpdf){
	pdf.SetFont("Arial", "", 10)
...
func RegimenfiscalTodosDetalle(pdf *gofpdf.Fpdf,miFila regimenfiscal, a int ){
	ene := pdf.UnicodeTranslatorFromDescriptor("")
	pdf.SetFont("Arial", "", 9)
	pdf.SetX(21)
	...
	pdf.CellFormat(40, 4, Subcadena(miFila.Codigo,0,12), "", 0,
		"L", false, 0, "")
	pdf.SetX(55)
	pdf.CellFormat(40, 4, ene(miFila.Nombre), "", 0,"", false, 0, "")

	pdf.Ln(4)
}

func RegimenfiscalTodosPdf(w http.ResponseWriter, r *http.Request) {
	db := dbConn()

	t := []regimenfiscal{}
	var e  empresa=ListaEmpresa()
	var c  ciudad=TraerCiudad(e.Ciudad)
	err := db.Select(&t, "SELECT * FROM regimenfiscal ORDER BY cast(codigo as integer) ")
	if err != nil {
		log.Fatalln(err)
	}
	var buf bytes.Buffer
	...
	pdf.SetHeaderFunc(func() {
		...
		pdf.CellFormat(190, 10, ene(c.NombreCiudad+ " - "+c.NombreDepartamento), "0", 0, "C", false, 0,
			"")
		pdf.Ln(6)
		pdf.CellFormat(190, 10, "DATOS REGIMEN FISCAL", "0", 0,
			"C", false, 0, "")
		pdf.Ln(10)
	})
	...
	RegimenfiscalTodosCabecera(pdf)
	// tercera pagina
	...
	for i, miFila := range t {
		...
		RegimenfiscalTodosDetalle(pdf,miFila,a)
	}
...
// DOCUMENTO EXCEL
func RegimenfiscalExcel(w http.ResponseWriter, r *http.Request) {
	db := dbConn()
	t := []regimenfiscal{}
	var e  empresa=ListaEmpresa()
	var c  ciudad=TraerCiudad(e.Ciudad)
	...
	f.SetCellValue("Sheet1", "A9","DATOS REGIMEN FISCAL")
